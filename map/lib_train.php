...
function train($username, $name, $data, $mapx, $mapy, $mapw, $bDebug=false, $platform)
{
	...
	$q = 'SELECT * FROM point WHERE placename = "'.DBFix($name).'"';
	...
	$q = 'insert into point values (now(), "'.DBFix($name).'", '.'"'.DBFix($username).'", '. $mapx.', '.$mapy.', '.$mapw;
