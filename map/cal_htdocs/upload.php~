	...
	} else
	{
		...
		if (!isset($_POST["username"]) || $_POST["username"] == "")
		{
			...
			} else
			{
				...
				DBConnect();
				$sql = "UPDATE users SET icon='" . DBFix($newFileName) . "' WHERE username='" . DBFix(base64_decode($_POST["username"])) . "'";
				$result = DB($sql);
...
if (isset($_POST["restoreDefault"]))
{
	...
	{
		DBConnect();
		$sql = "UPDATE users SET icon='' WHERE username='" . DBFix(base64_decode($_POST["username"])) . "'";
		$result = DB($sql);
